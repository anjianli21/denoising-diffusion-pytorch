...
def get_constraint_violation_car(x, c):
    ...
    car_start_pos = torch.tensor([[0.0, 10.0], [10.0, 10.0]])
    car_start_v = torch.tensor([0.0] * 2)
    car_start_theta = torch.tensor([7 * torch.pi / 4, 5 * torch.pi / 4])
    ...
    obs_num = 2
    car_num = 2
    car_v_bound = torch.tensor([-2.0, 2.0])
    car_goal_radius = torch.tensor(0.2)
    car_radius = torch.tensor(0.2)
    car_goal_pos = torch.tensor([[10.0, 0.0], [0.0, 0.0]])

    # V constraints
    # TODO: max v over trajectory violation? or each step v violation
    v_min_violation = torch.max(torch.tensor(0.0), car_v_bound[0] - state_v)
    v_min_violation = torch.sum(v_min_violation, dim=[1, 2])
    v_max_violation = torch.max(torch.tensor(0.0), state_v - car_v_bound[1])
    v_max_violation = torch.sum(v_max_violation, dim=[1, 2])

    # goal reaching constraints
    goal_reaching_violation = torch.zeros((batch_size, 2))
    for i in range(car_num):
        dist_to_goal_square = (state_x[:, i, -1] - car_goal_pos[i][0]) ** 2 + (state_y[:, i, -1] - car_goal_pos[i][1]) ** 2
        threshold = car_goal_radius ** 2
        goal_reaching_violation[:, i] = torch.max(torch.tensor(0.0), dist_to_goal_square - threshold)
    goal_reaching_violation = torch.sum(goal_reaching_violation, dim=1)

    # Obstacle avoidance constraints
    obstacle_avoidance_violation = torch.zeros((batch_size, car_num, obs_num, timestep + 1))
    for i in range(car_num):
        for j in range(obs_num):
            ...
            obstacle_avoidance_violation[:, i, j, :] = torch.max(torch.tensor(0.0), threshold - dist_to_obstacle_square)
    obstacle_avoidance_violation = torch.sum(obstacle_avoidance_violation, dim=[1, 2, 3])

    # Car avoidance constraints
    car_avoidance_violation = torch.zeros((batch_size, car_num, obs_num, timestep + 1))
    for i in range(car_num):
        for j in range(obs_num):
            if i != j:
                ...
                car_avoidance_violation[:, i, j, :] = torch.max(torch.tensor(0.0), threshold - dist_to_car_square)
...
def integrate_dynamics(x_sol, car_num, u_num_per_car, car_start_pos, car_start_v, car_start_theta, timestep, batch_size):
    ...
    car_control = torch.zeros((batch_size, car_num, timestep, u_num_per_car))
    for i in range(car_num):
    ...
    state_x = torch.zeros((batch_size, car_num, timestep + 1))
    state_y = torch.zeros((batch_size, car_num, timestep + 1))
    state_v = torch.zeros((batch_size, car_num, timestep + 1))
    state_theta = torch.zeros((batch_size, car_num, timestep + 1))
...
if __name__ == "__main__":
    ...
    x = torch.ones((512, 81))
    c = torch.ones((512, 6))
    violation = get_constraint_violation_car(x, c)

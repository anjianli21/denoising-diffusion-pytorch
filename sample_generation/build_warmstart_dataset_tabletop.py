...
def main():
    ...
    # Configure path
    input_obs_goal_output_time_control_parent_path = f"results/from_autodl/diffusion/tabletop/results/input_obs_goal_output_time_control_obj_6"
            ...
            for k in range(len(diffusion_w_list)):
                ...
                for l in range(len(condition_seed_list)):
                    ...
                    while not is_condition_reasonable:
                        ...
                        obs_radius = rng_condition.rand(4)
                        obs_pos = rng_condition.rand(4, 2)
                        car_goal_pos = np.array([[[1.0, 1.0], [1.0, 9.0], [9.0, 1.0], [9.0, 9.0]][rng_condition.randint(low=0, high=4)]])

                        parameters = {}
                        parameters["obs_radius"] = obs_radius * (OBS_RADIUS_MAX - OBS_RADIUS_MIN) + OBS_RADIUS_MIN
                        parameters["obs_pos"] = obs_pos * (OBS_POS_MAX - OBS_POS_MIN) + OBS_POS_MIN

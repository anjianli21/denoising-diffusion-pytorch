...
class GaussianDiffusion1D(nn.Module):
    ...
    def p_sample_loop(self, classes, shape, cond_scale=6., rescaled_phi=0.7):
        ...
        x_start = None
        img_list = []
        _, x_start = self.p_sample(img, 999, classes, cond_scale, rescaled_phi)
        img_list.append(self.unnormalize(img).detach().cpu())
        ...
        for t in tqdm(reversed(range(0, self.num_timesteps)), desc='sampling loop time step', total=self.num_timesteps):
            ...
            img_list.append(self.unnormalize(x_start).detach().cpu())

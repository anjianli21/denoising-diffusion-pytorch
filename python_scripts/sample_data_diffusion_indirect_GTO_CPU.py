...
def main(unet_dim,embed_class_layers_dims,timesteps,data_num,sample_num,thrust,diffusion_w,batch_size):
    ...
    checkpoint_path = f"/scratch/gpfs/jg3607/Diffusion_model/indirect_GTO/results/cr3bp_vanilla_diffusion_seed_0/unet_{unet_dim}_mults_{unet_dim_mults_in_str}_embed_class_{embed_class_layers_dims_in_str}_timesteps_{timesteps}_batch_size_{batch_size}_cond_drop_0.1_mask_val_0.0_train_data_{data_num}/"
    ...
    thrust_normalized = (thrust - min_thrust)/(max_thrust - min_thrust)
    ...
    min_costates = np.array([-2.0, -2.0,-2.0,-0.6694,-0.4905,-1.4456]) # rounded down to 4 digits after the point
    max_costates = np.array([2.0,2.0,0.1865,0.5033,0.3460,0.0404]) # rounded up to 4 digits after the point

    # Unnormalize costates
    ...
    if save_warmstart_data:
        parent_path = f"/home/jg3607/Thesis/Diffusion_model/denoising-diffusion-pytorch/results/generated_initializations/indirect_GTO/unet_{unet_dim}_mults_{unet_dim_mults_in_str}_embed_class_{embed_class_layers_dims_in_str}_timesteps_{timesteps}_batch_size_{batch_size}_cond_drop_0.1_mask_val_0.0"

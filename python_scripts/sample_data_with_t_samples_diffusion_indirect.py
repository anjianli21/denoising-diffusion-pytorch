...
def main(unet_dim,embed_class_layers_dims,timesteps,data_num,sample_num,thrust,diffusion_w,batch_size):
    ...
    # Add 0 initial coast
    full_solution = np.insert(full_solution,1,np.zeros([1001,sample_num]),axis=2)
    # Add 0 z costates
    full_solution = np.insert(full_solution,5,np.zeros([1001,sample_num]),axis=2)
    full_solution = np.insert(full_solution,8,np.zeros([1001,sample_num]),axis=2)
    # Add mass costate -1   
    full_solution = np.insert(full_solution,9,-np.ones([1001,sample_num]),axis=2)

    if save_warmstart_data:
        parent_path = f"/home/jg3607/Thesis/Diffusion_model/denoising-diffusion-pytorch/results/generated_initializations/indirect/unet_{unet_dim}_mults_{unet_dim_mults_in_str}_embed_class_{embed_class_layers_dims_in_str}_timesteps_{timesteps}_batch_size_{batch_size}_cond_drop_0.1_mask_val_0.0_with_all_timesteps_x0"
